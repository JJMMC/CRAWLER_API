...
def read_all_rows(tabla):
    conn = sql.connect("database/Old_data/rtr_db.db")
    cursor = conn.cursor()
...
def obtener_tablas(path="database/Old_data/rtr_db.db"):
...
def dict_of_tables_items(path="database/Old_data/rtr_db.db"):
    table_lst = obtener_tablas(path)
    data = {}
    data["Tablas"] = []
    for index in range(len(table_lst)):
...
def filter_precio_menor (tabla,precio):
    conn = sql.connect("database/database/Old_data/rtr_db.db")
    cursor = conn.cursor()
...
def check_if_db_exists(db_directory = "database/database/Old_data/rtr_db.db"):
...
def check_if_empty_db(db_directory = "database/database/Old_data/rtr_db.db"):
